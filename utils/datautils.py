...
def get_wikitext2(nsamples, seed, seqlen, tokenizer, eval_mode=True):
    if eval_mode:
        testdata = load_dataset('Salesforce/wikitext', 'wikitext-2-raw-v1', split='test', cache_dir="./datasets")
        # testdata = load_dataset('./datasets/wikitext', 'default', split='test')
        testenc = tokenizer("\n\n".join(testdata['text']), return_tensors='pt')
        return testenc.input_ids
    else:
        traindata = load_dataset('Salesforce/wikitext', 'wikitext-2-raw-v1', split='train', cache_dir="./datasets")
        # traindata = load_dataset('./datasets/wikitext', 'default', split='train')
...
def get_c4_new(nsamples, seed, seqlen, tokenizer, eval_mode=True):
    if eval_mode:
        valdata = load_dataset(
            'allenai/c4', data_files={'validation': 'en/c4-validation.00000-of-00008.json.gz'}, split='validation', cache_dir="./datasets"
        )
        ...
        traindata = load_dataset(
            'allenai/c4', data_files={'train': 'en/c4-train.00000-of-01024.json.gz'}, split='train', cache_dir="./datasets")
