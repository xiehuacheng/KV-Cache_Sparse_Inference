...
from intactkv.inference_model import inject_intactkv_inference_model, inject_quantized_kv_model, \
                                     inject_streaming_model, inject_h2o_model, inject_h2o_streaming_model
from eval_utils.ppl import ppl_eval
from eval_utils.mmlu import run_mmlu_eval
from new_eval.streaming_generate import streaming_eval
...
def parse_args():
    ...
    parser.add_argument("--tasks", type=str, choices=["ppl", "mmlu", "qa", "streaming"], default=None, help="eval task name")
    parser.add_argument("--batch_size", type=int, default=1, help="eval batch size")
    parser.add_argument("--mmlu_num_few_shots", type=str, default="5", help="#MMLU few shots")
    ...
    parser.add_argument("--quant_method", type=str, default="fp16", choices=["fp16", "fp32", "rtn", "gptq", "awq"],
    ...
    parser.add_argument("--enable_streaming_pos_shift", action="store_true")
    parser.add_argument("--start_size", type=int, default=8)
    parser.add_argument("--recent_size", type=int, default=1016)
    
    parser.add_argument("--enable_h2o", action="store_true")
    parser.add_argument("--k", type=int, default=512)
    parser.add_argument("--heavy_hitter_size", type=int, default=512)
    parser.add_argument("--heavy_hitter_recent_size", type=int, default=512)
...
def main():
    ...
    if args.quant_method == "fp16":
        ...
        args.output_dir = os.path.join(args.output_dir, "fp32", args.model_name)
    else:
    ...
    if args.kv_bits < 16 and not args.enable_streaming_pos_shift and not args.enable_h2o:
        # quantize KV cache
        model = inject_quantized_kv_model(args, model)
    else:
        if args.enable_streaming_pos_shift:
            if args.enable_h2o:
                model = inject_h2o_streaming_model(args, model)
            else:
                model = inject_streaming_model(args, model)
        else:
            if args.enable_h2o:
                model = inject_h2o_model(args, model)
            else:
                pass
    ...
    save_name = args.model_name + ("-streaming_pos_shift" if args.enable_streaming_pos_shift else "")
    save_name = save_name + ("-h2o" if args.enable_h2o else "")
    save_name = save_name + ("-intactkv" if args.intactkv else "")
    ...
    if args.tasks == "ppl":
        ...
        for dataset in datasets:
            ...
            ppl = ppl_eval(model.model, model.tokenizer, testloader, args)
            results[dataset] = ppl
    ...
    elif args.tasks == "streaming":
        streaming_eval(model)
        results = None
    else:
